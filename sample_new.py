...
n_timesteps = 1000
device = 'cuda'
model_path = './model_checkpoints_128_1/unet_epoch_51_celeba_128.pth'
n_sample = 16
save_dir = 'output_test_sample_V2'
...
def sample_nopipeline_ddim(net, noise_scheduler_ddim ,save_path="./tmp.jpg"):
    ...
    shape = (n_sample, *xt_shape)
    sample = torch.randn(shape).to(device)
    for i, t in enumerate(noise_scheduler_ddim.timesteps):
        with torch.no_grad():
            residual = net(sample, t).sample
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(save_dir, f'sampletest_{dataset_name}.jpg')
